...
# Instead, you can make global mappings so you'd only have to change it in one location
...
ggplot(data = diamonds) +
  geom_bar(mapping = aes(x = cut, color = cut)) # Has grey bars with a highlighted color around each bar

ggplot(data = diamonds) +
  geom_bar(mapping = aes(x = cut, fill = cut)) # Changes the color for each bar

ggplot(data = diamonds) +
  geom_bar(mapping = aes(x = cut, fill = clarity)) # Stacks the bars and has a different color for each clarity
